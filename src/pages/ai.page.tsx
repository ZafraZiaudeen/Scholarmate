...
export default function AITutorPage() {
 ...
 const handleSendMessage = async (): Promise<void> => {
   ...
   try {
     ...
     // Construct a structured AI response without correct answers or explanations
     let aiMessageContent = response.contentExplanation || "Here's your personalized learning task!";
     if (task.content?.bookContent) {
       aiMessageContent += `\n\n**Book Content:**\n- *Title*: ${task.content.bookContent.title}\n- *Chapter*: ${task.content.bookContent.chapter}\n- *Page*: ${task.content.bookContent.pageNumber}\n\n${task.content.bookContent.content}`;
     }
     if (task.content?.questions && task.content.questions.length > 0) {
       ...
       task.content.questions.forEach((q, index) => {
         ...
         q.options.forEach((option, oIndex) => {
           aiMessageContent += `- ${option.replace(/^\(\d+\)\s/, "")}\n`;
         });
